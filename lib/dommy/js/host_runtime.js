...
globalThis.__rbHost = (function () {
  ...
  function makeHandler(handle, methods, methodCache, arrayLike, named) {
    // Whether this proxy IS the global window (globalThis.window). In a browser
    // the window IS the global object, so the global window must share its
    // namespace with globalThis BOTH ways: a global a script creates via
    // `globalThis.X = …` (or a top-level `var` in a UMD bundle) reads back as
    // `window.X`, and a `window.X = …` write lands on globalThis so a later
    // bare `X` / `globalThis.X` resolves to it. Checked per trap miss (not
    // cached) — the proxy is built before it's installed as globalThis.window.
    // An iframe's contentWindow is NOT the global window and keeps its own
    // namespace.
    const isGlobalWindow = () => {
      const w = globalThis.window;
      return w !== undefined && proxyHandles.get(w) === handle;
    };
    ...
    return {
      get(t, prop, receiver) {
        ...
        // falls back to the shared JS global namespace, so e.g. a UMD bundle's
        // `globalThis.Stimulus = …` is visible as `window.Stimulus`.
        if (v === null && isGlobalWindow() && prop in globalThis) return globalThis[prop];
      ...
      set(t, prop, value, receiver) {
        ...
        // stash, …) keeps routing to the host below.
        if (isGlobalWindow() && rehydrate(__rb_host_get(handle, prop)) === null) {
      ...
      getOwnPropertyDescriptor(t, prop) {
        ...
        if (typeof prop !== "symbol" && isGlobalWindow() && Object.hasOwn(globalThis, prop)) {
      ...
      has(t, prop) {
        ...
        // The global window also reports JS globals (`"Stimulus" in window`).
        if (isGlobalWindow() && prop in globalThis) return true;
