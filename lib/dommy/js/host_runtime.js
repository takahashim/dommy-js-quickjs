...
globalThis.__rbHost = (function () {
  ...
  function protoForChain(chain, i) {
    ...
    // WebIDL [Constant]s on Node live on both the interface object and its
    // prototype (so `Node.ELEMENT_NODE`, `el.ELEMENT_NODE` and the inherited
    // `node.ELEMENT_NODE` all === the numeric value). Instances reach the
    // prototype copy via the proxy get trap's `prop in target` fallback.
    if (name === "Node") {
      for (const [k, val] of Object.entries(NODE_CONSTANTS)) {
  ...
  function attachStatics() {
    for (const name of protos.keys()) {
      ...
      for (const m of __rb_static_names(name)) {
        ...
        ctor[m] = (...args) => rehydrate(__rb_static_call(name, m, dehydrate(args)));
      }
    }
  }
