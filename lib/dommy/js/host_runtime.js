...
globalThis.__rbHost = (function () {
  ...
  function protoForChain(chain, i) {
    ...
    if (ARRAY_LIKE_COLLECTIONS.has(name)) {
      // The indexed-collection iterator protocol: keys()/values()/entries()
      // return real JS iterators (not arrays), reading length/[i] live through
      // the proxy. Defined JS-side so `list.keys() instanceof Array` is false.
      const arrayIterator = (self, kind) => {
        let i = 0;
        const it = {
          next() {
            if (i >= self.length) return { value: undefined, done: true };
            const idx = i++;
            const value = kind === "key" ? idx : kind === "entry" ? [idx, self[idx]] : self[idx];
      ...
      define(Symbol.iterator, function () { return arrayIterator(this, "value"); });
      define("values", function () { return arrayIterator(this, "value"); });
      define("keys", function () { return arrayIterator(this, "key"); });
      define("entries", function () { return arrayIterator(this, "entry"); });
  ...
  function makeHandler(handle, methods, methodCache, arrayLike) {
    ...
    return {
      ...
      getOwnPropertyDescriptor(t, prop) {
        if (arrayLike && isArrayIndex(prop) && Number(prop) < liveLength()) {
          return {
      ...
      ownKeys(t) {
        ...
        if (!arrayLike) return keys;
        const n = liveLength();
        const indices = [];
        for (let i = 0; i < n; i++) indices.push(String(i));
        // Indices first (spec enumeration order), then any non-index own keys
        // (expandos, symbols); drop target keys that collide with live indices.
        return indices.concat(keys.filter((k) => !(isArrayIndex(k) && Number(k) < n)));
      },
      has(t, prop) {
        // An out-of-range index on an array-like is genuinely absent (`2 in
        // nodeList` is false past its length); everything else stays permissive.
  ...
  function makeProxy(handle) {
    ...
    const p = new Proxy(target, makeHandler(handle, methods, new Map(), ARRAY_LIKE_COLLECTIONS.has(desc.name)));
