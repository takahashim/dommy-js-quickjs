...
globalThis.__rbHost = (function () {
  ...
  function makeProxy(handle) {
    ...
    const p = new Proxy(target, {
      get(t, prop, receiver) {
        ...
        // Let symbols (Symbol.toStringTag, Symbol.iterator) and JS-side
        // expandos (own props on the target) resolve directly — the latter
        // keeps object/instance fields with their identity intact.
        if (typeof prop === "symbol") return Reflect.get(t, prop, receiver);
        if (Object.hasOwn(t, prop)) return Reflect.get(t, prop, receiver);
        if (methods.has(prop)) {
          let fn = methodCache.get(prop);
          if (!fn) {
            fn = (...args) => rehydrate(__rb_host_call(handle, prop, dehydrate(args)));
        ...
        // (constructor, prototype methods like connectedCallback) the DOM ABI
        // doesn't provide.
      ...
      set(t, prop, value, receiver) {
        ...
        // A setter on the prototype chain (a framework's reactive property,
        // e.g. Lit) takes precedence over the DOM ABI.
        if (settersOf(Object.getPrototypeOf(t)).has(prop)) {
          Reflect.set(t, prop, value, receiver);
          return true;
        }
        // DOM property -> Dommy; anything Dommy doesn't recognise is kept as a
        // JS-side expando on the target (identity-preserving).
  ...
  return { makeProxy, invokeCallback, tag: dehydrate, interfaceOf, seedInterfaces, invokeLifecycle };
})();
