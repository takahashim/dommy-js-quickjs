...
(function () {
  ...
  class Observable {
    ...
    _subscribeWith(internalObserver, options) {
      ...
      if (outer) {
        ...
        } else if (typeof outer.__internalAddAbortAlgorithm === "function") {
          // Per spec, consumer unsubscription runs as an abort ALGORITHM on the
          // outer signal — before its "abort" event — so a subscription tears
          // down (and its own signal/abort fires) ahead of any external abort
          // listener (downstream-before-upstream ordering).
          outer.__internalAddAbortAlgorithm(() => subscriber._abortConsumer(outer.reason));
        } else {
    ...
    inspect(inspector) {
      ...
      return new Observable((subscriber) => {
        ...
        if (isCallable(cfg.abort)) {
          // Register as an abort ALGORITHM (before the abort event) and before
          // subscribing to the source — so the inspector's abort tap fires ahead
          // of the source subscription's own teardown.
          const tap = () => { try { cfg.abort(subscriber.signal.reason); } catch (_e) {} };
          if (typeof subscriber.signal.__internalAddAbortAlgorithm === "function") {
            subscriber.signal.__internalAddAbortAlgorithm(tap);
          } else {
            subscriber.signal.addEventListener("abort", tap, { once: true });
          }
        }
        source._subscribeWith({
          next: (v) => {
    ...
    toArray(options) {
      ...
      return new Promise((resolve, reject) => {
        ...
        const signal = consumerSignal(options, controller, reject);
  ...
  // The signal a promise-returning operator subscribes with: the operator's own
  // controller (so it can unsubscribe on resolve). When the caller's (outer)
  // signal aborts, reject the promise and abort that controller — as an abort
  // ALGORITHM (before the outer's abort event) so the subscription tears down
  // ahead of any external abort listener (downstream-before-upstream ordering).
  function consumerSignal(options, controller, reject) {
    const outer = options && options.signal;
    if (outer) {
      const onAbort = () => { reject(outer.reason); try { controller.abort(outer.reason); } catch (_e) {} };
      if (outer.aborted) onAbort();
      else if (typeof outer.__internalAddAbortAlgorithm === "function") outer.__internalAddAbortAlgorithm(onAbort);
