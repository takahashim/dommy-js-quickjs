...
## スナップショット (2026-05-30、URL + AttributeNS + createElement(NS) + iframe + classList の後)

```
  dom      1666/2261  (73.7%)
  url        91/109   (83.5%)
  total    1757/2370  (74.1%)   — 9 ファイルが完全グリーン
```

セッション開始時の 108/2370 (4.6%) からの伸び。内訳:
- URL コアの書き直し (下記) が `url` を 77.1%→83.5% に。`dom` には影響しない (url 専用)。
- Element `*AttributeNS` + Attr ノード一式が `attributes.html` を **7→47/67** に。
- **window interface コンストラクタ** + **createElement(NS) 検証** が createElementNS を
  **1→106/596**、createElement を **0→37/147** に。
- **iframe `contentDocument` ロード** が createElementNS を **106→316/596**、createElement
  を **37→58/147** に。
- **classList の void 戻り値 / 細部修正** (下記) が `Element-classlist.html` を
  **965→1235/1420** に (dom 1396→1666)。

残る `dom` の不足は createElementNS/createElement の残り (XML 文書セマンティクス、
null/undefined 強制変換、緩い旧ケース)、`attributes.html` の残り (47/67、own-property
列挙 + 重複属性 + libxml2 xmlns)、classList の残り (~185、MutationObserver レコード等)。
...
### `Element-classlist` の残り (1235/1420)
- void 戻り値 / item / toString / null 強制変換は **完了** (Landed 参照)。残り ~185 件は
  雑多なテール: `MutationObserver` レコードのタイミング/内容、`add`/`remove`/`toggle` の
  特定の空白・重複・順序ケース ("wrong class after modification")、`DOMTokenList` の
  イテレーション、外来名前空間 (SVG/MathML)。1 件あたりの ROI は低い。

### createElement / createElementNS (createElementNS 316/596, createElement 58/147)
window コンストラクタ + validate-and-extract / Name 検証 + iframe ロードで大きく前進
(Landed 参照)。残りは:
- **`null`/`undefined` の DOMString 強制変換** ("invalid qualified name: \"\"")。
  `createElementNS(ns, null)` は WebIDL DOMString 変換で `null`→`"null"`、`undefined`→
  `"undefined"` になるべきだが、ブリッジが JS null/undefined を両方 Ruby nil に畳むため
  `""` になり誤判定。null と undefined で異なる文字列が要るので、bridge の undefined
  区別 (後述) が前提。
- **XML/XHTML 文書セマンティクスの細部**。iframe 文書を `Dommy.parse` (HTML5) で起こして
  いるため厳密な XML 文書ではない。非 HTML 名前空間ケースは大文字化されず通るが、
  createElement (非 NS) を XML 文書で呼ぶと小文字化すべきでない等の差が残る。
- **vendored コーパスの緩い旧ケース** (~18 件)。`["http://example.com/", "0:a", null]`
  (prefix が数字始まり) や `"f:o:o"` (コロン 2 個) を valid 扱いする等、canonical QName より
  緩い古い WPT 期待値。canonical 実装 (および実ブラウザ/jsdom) でも落ちるため、追わない。
- `createElement(undefined)` の `HTMLUnknownElement` instanceof、要素プロパティの細部
  ("expected undefined got 1" 等)。
