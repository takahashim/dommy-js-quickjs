...
## スナップショット (2026-05-30、URL パーサー + AttributeNS + window コンストラクタの後)

```
  dom      1113/2261  (49.2%)
  url        91/109   (83.5%)
  total    1204/2370  (50.8%)   — 9 ファイルが完全グリーン
```
...
- Element `*AttributeNS` + Attr ノード一式が `attributes.html` を **7→47/67** に
  (dom 982→1022)。
- **window 上の interface コンストラクタ公開** (下記) が
  `Document-createElementNS.html` を **1→72/596**、`Document-createElement.html` を
  **0→20/147** に (dom 1022→1113)。

残る `dom` の不足の最大塊は `Document-createElementNS.html` (72/596) と
`Document-createElement.html` (20/147) の **iframe `contentDocument` = null** で
ブロックされた XML/XHTML 変種 (約 488 件、後述)。次いで雑多な `Element-classlist` の
テール (~965/1420)、`attributes.html` の残り (47/67)。
...
### createElement / createElementNS (createElementNS 72/596, createElement 20/147)
window コンストラクタ公開で大きく前進 (Landed 参照)。残りの最大ブロッカーは:
- **iframe `contentDocument` = null** (約 488 件)。両テストは各ケースを HTML / XML /
  XHTML の 3 document に対し実行するが、XML/XHTML 変種は `<iframe src=dummy.xml>` の
  `contentDocument` を使い、ハーネスが iframe サブドキュメントをロードしないため null →
  「documentElement of null」で全滅。**→ iframe サブドキュメント読込 (XML/XHTML パース
  + contentDocument 配線) という大きめのハーネス機能が必要。** これがコーパス全体で
  単一の最大レバー。
- **`null`/`undefined` の DOMString 強制変換** ("name must not be empty")。
  `createElement(null)` / `createElementNS(ns, null)` は WebIDL DOMString 変換で `null`→
  `"null"` になるべきだが、ブリッジが JS null を Ruby nil に畳むため `""` になり誤判定。
  [[undefined-null セクション]] と同根。
- **Name production 検証の厳しさ**。`NodeWrapperCache::NAME_RE` がユニコード/大文字の
  ローカル名 (`marK`, `İnput`, `ெfoo` 等) を誤って拒否; 緩和が必要。
- `createElementNS` の非 HTML 名前空間の大文字小文字保持 (`createElementNS(SVG, "svg").
  nodeName` は `"svg"` のまま; SVG/MathML の `tagName`)。
- `createElement(undefined)` は `HTMLUnknownElement` のシードを期待 (instanceof)。
