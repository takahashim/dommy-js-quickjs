...
### createElement / createElementNS (createElementNS 486/596, createElement 58/147)
window コンストラクタ + 検証 + iframe + 名前空間メタデータ + Node 定数で大きく前進
(Landed 参照)。残りは:
- **`null`/`undefined` の DOMString 強制変換** ("invalid qualified name: \"\"", ~21 件)。
  ...
  区別 (後述) が前提。
- **vendored コーパスの緩い旧ケース** (~30 件)。`["http://example.com/", "0:a", null]`
  (prefix が数字始まり) や `"f:o:o"` (コロン 2 個) を valid 扱いする等、canonical QName より
  緩い古い WPT 期待値。canonical 実装 (および実ブラウザ/jsdom) でも落ちるため、追わない。
- **createElement (非 NS、58/147)**: createElement は HTML 文書では Name production 検証
  + 小文字化。残りは Name 検証の細部、`createElement(undefined)` の `HTMLUnknownElement`
  instanceof、XML 文書での createElement (iframe 文書は HTML5 パースなので小文字化が
  入る) 等。
